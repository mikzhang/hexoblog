...
![snipaste_20190927143803.jpg](snipaste_20190927143803.jpg)
...
![snipaste_20190927144353.jpg](snipaste_20190927144353.jpg)
...
![snipaste_20190927144620.jpg](snipaste_20190927144620.jpg)

有通常情况就有**特殊情况**，这里的特殊是指：

![snipaste_20190927144721.jpg](snipaste_20190927144721.jpg)
...
1. 在编译生成class文件时，会自动产生两个方法，一个是类的初始化方法
2. , 另一个是实例的初始化方法
clinit>：在jvm第一次加载class文件时调用，包括静态变量初始化语句和静态块的执行
3. <init>: 在实例创建出来的时候调用，包括调用new操作符；调用 Class 或 Java.lang.reflect.Constructor 对象的newInstance()方法；调用任何现有对象的clone()方法；通过 java.io.ObjectInputStream 类的getObject() 方法反序列化。
...
![snipaste_20190927150534.jpg](snipaste_20190927150534.jpg)

那么去掉报错的那句，改成下面：

![snipaste_20190927150616.jpg](snipaste_20190927150616.jpg)
输出结果：1
...
![snipaste_20190927150833.jpg](snipaste_20190927150833.jpg)
...
![snipaste_20190927151007.jpg](snipaste_20190927151007.jpg)

输出结果：无

**验证 3**: 常量在编译阶段会存入调用类的常量池中，本质上并没有直接引用到定义常量的类，因此不会触发定义常量的类的初始化

![snipaste_20190927151054.jpg](snipaste_20190927151054.jpg)
输出结果：hello world
...
![snipaste_20190927151943.jpg](snipaste_20190927151943.jpg)
...
```
我们发现除了启动类加载器（BootStrap ClassLoader），每个类都有其"父类"加载器. **其实这里的父子关系是组合模式，不是继承关系来实现**

![snipaste_20190927152141.jpg](snipaste_20190927152141.jpg)

从图中可以看到类 AppClassLoader 和 ExtClassLoader 都继承 URLClassLoader， 而 URLClassLoader 又继承 ClassLoader， 在 ClassLoader 中有一个属性

![snipaste_20190927152230.jpg](snipaste_20190927152230.jpg)

在通过构造函数实例化  AppClassLoader 和 ExtClassLoader 的时候都要传入一个 classloader 作为当前 classloader 的 parent

![snipaste_20190927152318.jpg](snipaste_20190927152318.jpg)
...
```
![snipaste_20190928173406.jpg](snipaste_20190928173406.jpg)
```
把字节数组 b中的内容转换成 Java 类，其开始偏移为off,这个方法被声明为final的
```
![snipaste_20190928173500.jpg](snipaste_20190928173500.jpg)
```
查找指定名称的类
```
![snipaste_20190928173545.jpg](snipaste_20190928173545.jpg)
```
链接指定的类
```
![snipaste_20190928173643.jpg](snipaste_20190928173643.jpg)
...
![snipaste_20190928173942.jpg](snipaste_20190928173942.jpg)
...
![snipaste_20190928174056.jpg](snipaste_20190928174056.jpg)
...
![snipaste_20190928174207.jpg](snipaste_20190928174207.jpg)
...
![snipaste_20190928174410.jpg](snipaste_20190928174410.jpg)

进入上面第25行的 loadClass 方法中

![snipaste_20190928174525.jpg](snipaste_20190928174525.jpg)
...
![snipaste_20190928174605.jpg](snipaste_20190928174605.jpg)

借用网友的一个加载时序图来解释整个过程更加清晰：

![snipaste_20190928174653.jpg](snipaste_20190928174653.jpg)
...
![snipaste_20190928174739.jpg](snipaste_20190928174739.jpg)

然后提供了一个DriverManager来管理这些Driver的具体实现：

![snipaste_20190928174813.jpg](snipaste_20190928174813.jpg)
...
![snipaste_20190928174909.jpg](snipaste_20190928174909.jpg)

核心就是这句Class.forName()触发了mysql驱动的加载，我们看下mysql对Driver接口的实现：

![snipaste_20190928174946.jpg](snipaste_20190928174946.jpg)
...
![snipaste_20190928175029.jpg](snipaste_20190928175029.jpg)

可以看到这里直接获取连接，省去了上面的Class.forName()注册过程。
现在，我们分析下看使用了这种spi服务的模式原本的过程是怎样的:

1. 第一，从META-INF/services/java.sql.Driver文件中获取具体的实现类名“com.mysql.jdbc.Driver”
2. 第二，加载这个类，这里肯定只能用class.forName("com.mysql.jdbc.Driver")来加载
...
![snipaste_20190928175303.jpg](snipaste_20190928175303.jpg)

继续向下看构造函数实例化 ServiceLoader 做了哪些事情：

![snipaste_20190928175411.jpg](snipaste_20190928175411.jpg)

查看 reload() 函数：

![snipaste_20190928175447.jpg](snipaste_20190928175447.jpg)

继续查看LazyIterator构造器，该类同样实现了Iterator接口：

![snipaste_20190928175525.jpg](snipaste_20190928175525.jpg)

实例化到这里我们也将上下文得到的类加载器实例化到这里，来回看ServiceLoader 重写的 iterator() 方法：

![snipaste_20190928175614.jpg](snipaste_20190928175614.jpg)

上面next() 方法调用了lookupIterator.next()，这个lookupIterator 就是刚刚实例化的 LazyIterator(); 来看next方法

![snipaste_20190928175643.jpg](snipaste_20190928175643.jpg)

继续查看nextService 方法：

![snipaste_20190928175724.jpg](snipaste_20190928175724.jpg)
...
https://mp.weixin.qq.com/s?__biz=Mzg3NjIxMjA1Ng==&mid=2247483821&idx=1&sn=4b51ef0930bd884f13f5c2ab29951edc&scene=21#wechat_redirect
